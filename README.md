...
改写 AIST 窗函数（`f_inc` / `f_dec` 分向 + 越界保护），解决 Ron/Roff 边界锁死——长时序 NAL/EN 联调的前提条件。  
→ [技术专节](#忆阻模型改进)

#### 共存检测 + 方向性突触 · `独立提出`

在 EN 与 MMAM 之间新建 CD/AndCD 子电路：异步脉冲下正确判定「共现」，并按 a→b / b→a 非对称写入联想权重。  
→ [技术专节](#共存检测与方向性突触)

**系统集成与验证**

- **全链路可仿真** — 三通道 OrCAD 工程，SJ → NAL → EN → CD → MMAM 完整数据通路
- **多机制统一验证** — NAL 四过程 · MSE/MSD 双向整合 · 跨模态 VR 检索 · 痛觉 / 语义饱和扩展
- **过程可回溯** — 128 / 204 组会、设计笔记与多版 PPT 记录完整迭代路径

**主要结论** — 在同一套忆阻神经形态电路上完整跑通前端学习调制、多感官双向整合与方向性联想写入；配套论文终稿、OrCAD 工程与仿真数据，可直接复现。
...
| 关注点 | 可从这些章节入手 |
|--------|------------------|
| 项目概览 | [核心信息卡](#核心信息卡) → [技术栈](#技术栈) → [主要仿真结果](#-主要仿真结果) |
