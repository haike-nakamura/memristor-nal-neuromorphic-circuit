...
- 💡 **含金量**：两大改动（忆阻模型 + 共存检测）均为**独立提出并实现**，配套完整 OrCAD 工程、论文与仿真数据，可直接复现。
- 📂 **仓库含什么**：论文终稿 · 全部阶段 PPT · 三通道电路工程 · 仿真波形 CSV · 本 README 图文说明。

---

## 📖 术语与关键词

下文将反复出现若干专业名词。本节先统一诠释，后文不再重复解释。

### 核心概念

| 术语 | 英文 | 通俗解释 |
|------|------|----------|
| **忆阻器** | Memristor | 阻值随电压历史变化的电子元件，可模拟生物突触的「记忆」与可塑性，是本电路的基本器件 |
| **神经形态电路** | Neuromorphic Circuit | 模仿生物神经系统结构与信息处理方式的电子电路，用硬件实现学习、记忆等行为 |
| **非联想学习** | Non-Associative Learning (NAL) | 生物最基础的学习形式：对单一刺激的适应与调制，包括习惯化、敏感化、去习惯化、自发恢复 |
| **多感官整合** | Multisensory Integration (MSI) | 大脑整合视觉、听觉等多通道信号的机制；本电路实现其增强（MSE）与抑制（MSD）两个方向 |
| **共存检测** | Coexistence Detection (EC) | **本项目独立设计**：判定两路脉冲是否在有效时间窗内「共现」，作为是否建立联想的 Hebbian 前提 |
| **方向性突触** | Directional Synapse | **本项目独立设计**：区分 a→b 与 b→a 的非对称联想写入，而非简单平均两路信号 |
| **赫布学习** | Hebbian Learning | 「一起发放，一起连接」——仅当两路神经活动共现时，才强化突触连接 |

### 电路模块缩写

| 缩写 | 全称 | 在系统中的角色 |
|------|------|----------------|
| **SJ** | Stimulus Judgment | 刺激判断：分类刺激强度，判断有害/无害 |
| **NAL** | Non-Associative Learning | 非联想学习：习惯化 / 敏感化 / 去习惯化 / 自发恢复 |
| **EN** | Encoding Neuron | 编码神经元：将电压强度转为脉冲频率信号 |
| **EC** | Coexistence Detection | 共存检测：判定两通道脉冲是否时域重叠 |
| **AndEC** | AND + EC Gate | 方向性门控：共现成立时，才允许对应方向的突触电压写入忆阻 |
| **MMAM** | Multisensory Mutual Associative Memory | 多感官互联记忆：忆阻交叉阵列实现跨通道联想存储与检索 |
| **MSE** | Multisensory Enhancement | 多感官增强：共现刺激强化关联 |
| **MSD** | Multisensory Depression | 多感官抑制：非共现刺激削弱无关关联 |
| **VR** | Retrieval | 检索输出：单通道线索唤醒关联通道的脉冲 |
| **VM** | — | 通道使能 / 生理响应输出 |
...
| 关键词 | 含义 |
|--------|------|
| **VNE** | 编码后的原生神经元脉冲，用于重叠判定 |
| **VPERM** | 覆盖整段刺激期的使能窗，用于门控共现输出 |
| **VENG** | 共现成立后的统一驱动脉冲（3 V） |
| **M12 / M21 …** | 方向性联想忆阻（如 M12 = 通道 1→2 的突触权重） |
| **AIST 模型** | 基于硫系化合物实验数据的阈值忆阻 SPICE 模型 |
| **Biolek 窗函数** | 限制忆阻状态变量变化范围的数学函数；本项目做了分向改进 |
| **Ron / Roff** | 忆阻最低阻 / 最高阻，触界后原始模型会锁死 |
...
| 你关心什么 | 建议阅读 |
|------------|----------|
| 👀 快速了解价值 | [主要仿真结果](#-主要仿真结果) → [核心贡献](#-核心贡献) → [个人工作边界](#-个人工作边界) |
| 🔬 技术评审 | [忆阻模型改进](#忆阻模型改进) → [共存检测与方向性突触](#共存检测与方向性突触) → [仿真结果](#仿真结果) |
| 🛠️ 复现工程 | [环境配置](#环境配置) → [`memtest/`](memtest/) · [`memristor/`](memristor/) → [`simulation/`](simulation/) |
...
本项目实现了一套**可仿真的三通道忆阻神经形态电路**：在单块电路中串联 **NAL → EN → EC → MMAM**，覆盖习惯化/敏感化等前端调制、多感官增强与抑制（MSE/MSD）、跨模态检索（VR）及痛觉感受器、语义饱和等扩展场景。
...
| 异步脉冲下与门无法判定共现，突触无方向性 | **共存检测 + 方向性突触**（EC/AndEC） |
...
- 🔌 **全链路可仿真** — OrCAD + PSpice 三通道系统，SJ → NAL → EN → EC → MMAM 完整数据通路
- 🧩 **两大核心改动** — 忆阻模型边界锁死修复 + 共存检测子电路（均为独立贡献）
- ✅ **功能完整性** — NAL 四过程 + MSE/MSD 双向整合 + 方向性跨模态检索，同板验证
- 📦 **可复现归档** — 论文终稿、全部阶段 PPT、OrCAD 工程、仿真 CSV 与绘图脚本
...
| 2 | **共存检测 + 方向性突触** | **`独立提出`** | 时域 Hebbian 前提判定（EC/AndEC），区分 a→b 与 b→a，非对称驱动忆阻阵列 |
...
> 供评审快速区分：**哪些是我的原创设计与实现，哪些是参考框架上的工程扩展。**

| 标签 | 含义 | 具体内容 |
|:----:|:----:|----------|
| ![独立提出](https://img.shields.io/badge/独立提出-原创设计%2F实现-2ea44f?style=flat-square) | **原创** | 共存检测子电路（EC/AndEC）；方向性忆阻驱动（M12/M21/…）；Biolek 窗函数分向改进；语义饱和方向性联想方案 |
